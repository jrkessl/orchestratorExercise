...
# clear results file
if [[ -e ~/Documents/gitlab-viapath/result-release ]]; then
    rm ~/Documents/gitlab-viapath/result-release
fi
...
for folder in $( ls -alF | grep -E ^d | grep -v -E -- "\.\/" |  grep -v helm-command | grep -v helm-oms | awk '{print $9}' ); do # for all the projects (for all the lines returned by this statement)
    ...
    # test for branches like release/nnn.n.n
    for rb in $( git branch -r | grep -E -- "release\/2[0-9]{2}\.[0-9]{1,3}\.[0-9]{1,3}$" | sort | sed 's/origin\///' | tail -n 3 ); do
        ...
        if [[ $error -eq 1 ]]; then
            ...
            # echo " ---------------------- Project ${folder}, does not have a qa branch. " | tee -a ~/Documents/gitlab-viapath/result-release
            ...
            if [[ -s /tmp/tmp ]]; then
                ...
                forty="                                        " 
                folder="${folder:0:40}${forty:0:$((40 - ${#folder}))}"
                echo " ---------------------- Project ${folder}, branch ${currentbranch}, these files still use docker: " | tee -a ~/Documents/gitlab-viapath/result-release
                echo "${files}" | tee -a ~/Documents/gitlab-viapath/result-release
                rm /tmp/tmp
            else    
                # string padding to 40 chars lenght
                forty="                                        " 
                folder="${folder:0:40}${forty:0:$((40 - ${#folder}))}"
                echo " ---------------------- Project ${folder}, branch ${currentbranch}, is clear. " | tee -a ~/Documents/gitlab-viapath/result-release
            fi
        fi
    done
    # if there was no release/nnn.n.n branches, let's try just 'release'. 
    if [[ $releasebranches -eq 0 ]]; then

        echo "No release/nnn.n.n? So lets try just 'release'."
        ((releasebranches++)) || echo "duh"
        #echo "Found a release branch: ${rb}. Lets check it out."
        rb=release
        echo "will run 'git checkout ${rb}'..."
        ...
        if [[ $error -eq 1 ]]; then
            ...
            forty="                                        " 
            folder="${folder:0:40}${forty:0:$((40 - ${#folder}))}"
            echo " ---------------------- Project ${folder}, does not have a ${rb} branch. " | tee -a ~/Documents/gitlab-viapath/result-release
        else
            currentbranch=$(git rev-parse --abbrev-ref HEAD)
            echo "we are in ${currentbranch}"
            echo "git pull ..." 
    ...
    # We did what we could. Were there really no release branches?
    if [[ $releasebranches -eq 0 ]]; then
        echo "Whoops! Looks like project ${folder} does not have any matching release branches."
        # string padding to 40 chars lenght
        forty="                                        " 
        folder="${folder:0:40}${forty:0:$((40 - ${#folder}))}"
        echo " ---------------------- Project ${folder} does not have any matching release branches. " | tee -a ~/Documents/gitlab-viapath/result-release
...
#cat ~/Documents/gitlab-viapath/result-release
